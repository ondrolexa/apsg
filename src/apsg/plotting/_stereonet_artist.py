...
class StereoNet_Scatter(StereoNet_Artists):
    ...
    def parse_kwargs(self, kwargs):
        ...
        if self.kwargs["s"] is not None:
            ...
            if nof != nos:
                raise TypeError(f'Number of sizes ({nos}) do not match number of features ({nof})')
        if self.kwargs["c"] is not None:
            ...
            if np.vstack(self.args).shape[0] != len(self.kwargs["c"]):
                raise TypeError(f'Number of colors ({noc}) do not match number of features ({nof})')
...
class StereoNet_Cone(StereoNet_Artists):
    ...
    def parse_kwargs(self, kwargs):
        ...
        if np.vstack(self.args).shape[0] != len(self.kwargs["angles"]):
            raise TypeError(f'Number of angles ({noa}) do not match number of features ({nof})')
        if not isinstance(self.kwargs["label"], str):
            if len(self.args) == 1:
                if issubclass(type(self.args[0]), Vector3):
                    self.kwargs["label"] = f"Cone {self.args[0].label()} ({self.kwargs['angles'][0]})"
                else:
    ...
    def parse_kwargs(self, kwargs):
        ...
        if not isinstance(self.kwargs["label"], str):
            if len(self.args) == 1:
                ...
                self.kwargs["label"] = f"Fault ({len(self.args)})"
        self.kwargs = np.copysign(1, np.atleast_1d(kwargs.get("sense", 1))).astype(int).tolist()
